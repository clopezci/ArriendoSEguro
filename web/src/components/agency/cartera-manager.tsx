...
/** Estado visible: si el contrato ya está firmado, manda sobre el estado de agencia. */
function derived(c: { agencyStatus: Contract["agencyStatus"]; contractStatus: string }): Contract["agencyStatus"] {
  if (c.contractStatus === "signed") return "signed";
  return c.agencyStatus === "sent" ? "sent" : "draft";
}
...
export function CarteraManager({ agencyId }: { agencyId: string }) {
  ...
  async function renew(c: Contract) {
    const input = window.prompt(
      `Renovar contrato de ${c.tenantName}.\nCanon nuevo en pesos (deja vacío para ajustar por IPC legal):`,
      "",
    );
    if (input === null) return; // canceló
    const monthlyRent = input.trim() ? Math.floor(Number(input.replace(/[^\d]/g, ""))) : undefined;
    setSending(true);
    setSendMsg(null);
    setSendErr(null);
    try {
      const res = await fetch(`/api/agency/${agencyId}/contracts/${c.contractId}/renew`, {
        method: "POST",
        headers: { "content-type": "application/json", ...(await buildAuthHeaders(user)) },
        body: JSON.stringify(monthlyRent ? { monthlyRent } : {}),
      });
      const json = (await res.json()) as { success?: boolean; monthlyRent?: number; errors?: { message?: string }[] };
      if (!res.ok || !json.success) setSendErr(json.errors?.[0]?.message ?? "No se pudo renovar.");
      else {
        setSendMsg(`✅ Renovación generada como borrador (canon ${money(json.monthlyRent ?? 0)}). Revísala y envíala a firma.`);
        await load();
      }
    } catch {
      setSendErr("Error de red al renovar.");
    } finally {
      setSending(false);
    }
  }
  ...
  const counts = {
    draft: rows.filter((r) => derived(r) === "draft").length,
    sent: rows.filter((r) => derived(r) === "sent").length,
    signed: rows.filter((r) => derived(r) === "signed").length,
  };
      ...
      {counts.draft > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-violet-200 bg-violet-50/40 p-3">
          <button
            type="button"
            disabled={sending}
            onClick={() => void sendSignatures(rows.filter((r) => derived(r) === "draft").map((r) => r.contractId))}
              ...
              {rows.map((c) => (
                <tr key={c.contractId} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-2">
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${STATUS[derived(c)].cls}`}>{STATUS[derived(c)].label}</span>
                  </td>
                  <td className="px-3 py-2">
                    <div className="flex gap-2">
                      ...
                      {derived(c) === "signed" && (
                        <button type="button" disabled={sending} onClick={() => void renew(c)} className="rounded-md border border-emerald-300 px-3 py-1 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-40">Renovar</button>
                      )}
                    </div>
                  </td>
                </tr>
