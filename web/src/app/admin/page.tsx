...
/** Pestaña PITCH: elevator pitch / plan de negocio con tablas, cifras y gráficas. */
function PitchTab({ s }: { s?: DashboardPayload["summary"] }) {
  const cop = (v: number | null | undefined) => (v == null ? "—" : `$${Number(v).toLocaleString("es-CO")}`);
  const lean = s?.lean;

  const market = [
    { k: "TAM — hogares en arriendo (Colombia)", v: 7_200_000, color: "#6366f1", note: "DANE ECV 2023 (40,3% de 18M)" },
    { k: "SAM — arriendo informal/directo (nuestro foco)", v: 3_600_000, color: "#0ea5e9", note: "≈ la mitad (probablemente más)" },
    { k: "SOM — meta aspiracional (~30% del SAM)", v: 1_080_000, color: "#10b981", note: "largo plazo; hitos cercanos menores" },
  ];
  const maxMarket = market[0].v;
  ...
  const PRICE = 49_900, MARGIN = 0.88, FIXED = 300_000;
  const rows = scenarios.map((sc) => {
    const ingreso = sc.contracts * PRICE;
    const bruto = Math.round(ingreso * MARGIN);
    const utilidad = bruto - FIXED;
    return { ...sc, ingreso, bruto, utilidad };
  });
  const maxUtil = Math.max(1, ...rows.map((r) => r.utilidad));

  const competitors = [
    ["Inmobiliarias tradicionales", "Administran cobrando comisión mensual", "Caras; no sirven al arrendador directo; poco digitales"],
    ["Generadores de contratos", "Generan el documento", "Solo el contrato; sin firma+pagos+reputación+cierre"],
    ["Marketplaces (Houm, etc.)", "Publican y conectan inmuebles", "Buscan inquilino, no gestionan el ciclo"],
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border-2 border-violet-400 bg-gradient-to-r from-violet-50 to-white p-5">
        ...
        <p className="mt-1 text-lg font-extrabold text-slate-900">
          Digitalizamos TODO el ciclo del arriendo directo —contrato legal, firma, pagos, inventario, reputación y cierre— fácil, con IA y a un costo mínimo ($49.900).
        </p>
        <p className="mt-1 text-sm text-slate-600">
          7,2M hogares arriendan en Colombia y la mayoría lo hace informal, sin respaldo. Democratizamos lo que hoy solo tienen las inmobiliarias.
          <b> La app ya está construida:</b> no pedimos plata para desarrollar, sino para <b>marketing</b> y sostener las plataformas.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ["1. Todo en uno (extremo a extremo)", "Del contrato al cierre, sin saltar entre apps/abogados/inmobiliarias.", "#6366f1"],
            ["2. Fácil + flujo progresivo + IA", "Una pregunta a la vez, guiado, con IA y voz; usable por adultos mayores.", "#0ea5e9"],
            ["3. Muy bajo costo — democratizar", "$49.900 por contrato vs. ~1 mes de canon de una inmobiliaria.", "#10b981"],
          ].map(([t, d, c]) => (
            <div key={t} className="rounded-xl border bg-white/95 p-3" style={{ borderColor: c }}>
              <p className="text-sm font-bold" style={{ color: c }}>{t}</p>
        ...
        <div className="mt-3 space-y-2">
          {market.map((m) => (
            <div key={m.k} className="flex items-center gap-2 text-xs">
              <span className="w-56 shrink-0 text-slate-700">{m.k}</span>
              {bar(m.v, maxMarket, m.color)}
              <span className="w-20 shrink-0 text-right font-bold tabular-nums">{(m.v / 1_000_000).toLocaleString("es-CO")} M</span>
            </div>
          ))}
        </div>
        <p className="mt-2 text-[11px] text-slate-500">Solo <b>1% del SAM (~36.000 contratos)</b> × $49.900 ≈ <b>$1.795 M COP</b>. Fuente: DANE ECV 2023.</p>
      </div>

      {/* Cómo vamos HOY (KPIs en vivo) */}
      {lean && (
        <div className="rounded-xl border border-emerald-300 bg-emerald-50/40 p-4">
          ...
          <div className="mt-2 grid gap-3 sm:grid-cols-4">
            {[
              ...
              ["Compras", String(lean.revenue.count)],
            ].map(([l, v]) => (
              <div key={l} className="rounded-lg border border-emerald-200 bg-white/80 p-3 text-center">
                <p className="text-[11px] uppercase tracking-wide text-slate-500">{l}</p>
                <p className="mt-1 text-lg font-bold text-emerald-800">{v}</p>
          ...
          <p className="mt-2 text-[11px] text-slate-500">Evidencia real y actualizada — ver pestaña <b>📈 Lean</b> para AARRR, cohortes y motores de crecimiento.</p>
        </div>
      )}
      ...
      <div className="rounded-xl border border-slate-300 bg-white/95 p-4">
        ...
        <div className="mt-2 overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {competitors.map((c) => (
                <tr key={c[0]} className="border-t border-slate-100 align-top">
                  <td className="py-1 pr-3 font-semibold">{c[0]}</td>
                  <td className="py-1 pr-3 text-slate-600">{c[1]}</td>
                  <td className="py-1 text-slate-800">{c[2]}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="mt-2 text-[11px] text-slate-500"><b>Moat:</b> reputación privada (datos propios) + motor legal Ley 820/527 + costo marginal casi cero.</p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-300 bg-white/95 p-4">
          ...
          <ul className="mt-2 space-y-1 text-xs text-slate-700">
            <li className="flex justify-between"><span>Precio por contrato</span><b>{cop(PRICE)}</b></li>
            <li className="flex justify-between"><span>Costo variable / contrato (est.)</span><b>~$3.000–6.000</b></li>
            <li className="flex justify-between"><span>Margen bruto</span><b>~88%</b></li>
            <li className="flex justify-between"><span>Desarrollo y mantenimiento</span><b className="text-emerald-700">$0 (fundador)</b></li>
          </ul>
        </div>
        <div className="rounded-xl border border-slate-300 bg-white/95 p-4">
          <p className="text-sm font-semibold text-slate-900">Costos de plataforma (est. / mes)</p>
          <ul className="mt-2 space-y-1 text-xs text-slate-700">
            <li className="flex justify-between"><span>Vercel (hosting)</span><b>~$80.000</b></li>
            <li className="flex justify-between"><span>Firebase (DB/Storage/Auth)</span><b>$0–100.000</b></li>
            <li className="flex justify-between"><span>Resend (correo)</span><b>$0–80.000</b></li>
            <li className="flex justify-between"><span>WhatsApp/Meta + IA</span><b>variable</b></li>
            <li className="flex justify-between border-t border-slate-100 pt-1"><span>Total inicial</span><b>~$160k–$500k</b></li>
          </ul>
          <p className="mt-1 text-[11px] text-slate-400">Estimado — ajustar con cifras reales.</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-300 bg-white/95 p-4">
        ...
        <p className="text-[11px] text-slate-500">Precio $49.900 · margen ~88% · costos fijos ~$300.000/mes. Utilidad = margen − fijos.</p>
        <div className="mt-3 space-y-2">
          {rows.map((r) => (
            <div key={r.k} className="flex items-center gap-2 text-xs">
              <span className="w-24 shrink-0 font-semibold text-slate-700">{r.k}</span>
              <span className="w-20 shrink-0 tabular-nums text-slate-500">{r.contracts}/mes</span>
        ...
        <p className="mt-2 text-[11px] text-slate-500"><b>Break-even ~7 contratos/mes</b> (dev/mantenimiento en $0 → quema mínima). Cifras ilustrativas; validar con el tablero.</p>
      </div>
      ...
      <div className="rounded-xl border-2 border-violet-300 bg-violet-50/50 p-4">
        ...
        <p className="mt-1 text-xs text-slate-700">
          Fondos para <b>marketing</b> (hoy incipiente) + <b>colchón de costos de plataforma</b> durante el crecimiento (opcional: alianzas legales/seguros).
          <b> No</b> para desarrollo ni nómina técnica. Con margen ~88% y break-even ~7 contratos/mes, cada peso de marketing eficiente se vuelve margen; el LTV/CAC del tablero guía la reinversión.
        </p>
      </div>

      <p className="text-[11px] text-slate-400">
        Cifras de mercado: DANE ECV 2023 (7,2M hogares en arriendo, 40,3%). Documento completo y reutilizable en <code>docs/MODULO-INDICADORES-LEAN.md</code> (Bloque 2). Los números marcados como estimados deben validarse con datos reales.
      </p>
    </div>
  );
}
