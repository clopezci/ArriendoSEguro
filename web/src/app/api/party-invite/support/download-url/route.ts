...
/** URL firmada de lectura de un documento subido, solo para el DUEÑO que invitó. */
export async function GET(request: Request) {
  ...
  const doc = await firestore.collection(PARTY_INVITE_SUPPORTS_COLLECTION).doc(id).get();
  const data = doc.data() as { inviterUid?: string; storagePath?: string; fileName?: string } | undefined;
  if (!doc.exists || !data || data.inviterUid !== auth.user.uid) {
