...
type ResponseItem = { action: string; reason: string | null; at: string };
type Req = {
  id: string;
  title: string;
  description: string;
...
const STATUS_META: Record<string, { label: string; cls: string }> = {
  open: { label: "Abierta (esperando al dueño)", cls: "border-sky-300 bg-sky-50 text-sky-800" },
  accepted: { label: "Aceptada", cls: "border-emerald-300 bg-emerald-50 text-emerald-800" },
  rejected: { label: "Rechazada", cls: "border-rose-300 bg-rose-50 text-rose-800" },
...
export default function MantenimientoPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const j = (await res.json()) as { success?: boolean; myRole?: string; requests?: Req[]; errors?: { message?: string }[] };
      if (!res.ok || !j.success) {
        setError(j.errors?.[0]?.message ?? "No se pudieron cargar las reparaciones.");
        return;
      }
      setMyRole(j.myRole ?? "");
  ...
  // Aliado jurídico (si hay), para ofrecerlo cuando algo entra en disputa.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
  ...
  async function respond(reqId: string, action: "accept" | "reject", reason?: string) {
    if (!user) return;
    try {
    ...
    <div className="mx-auto max-w-3xl p-4 sm:p-6">
      <WizardShell title="Reparaciones y mantenimiento" currentStep={13} contractId={id} variant="extra" phase="posventa">
        <p className="mb-4 text-sm text-slate-700">
          ...
          aliado. ArriendoSeguro no ejecuta las reparaciones; da trazabilidad y comunicación.
        </p>

        <RequiresSavedContract id={id}>
          {!user && (
            <p className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
              Inicia sesión con el correo de una parte del contrato para ver y registrar reparaciones.
            </p>
          )}
          {error && <p className="rounded-2xl border border-rose-300 bg-rose-50 p-3 text-sm text-rose-800">{error}</p>}

          {/* Nueva solicitud (cualquier parte puede reportar; típicamente el inquilino). */}
          {user && (
            <section className="rounded-3xl border-2 border-slate-200 bg-white/90 p-5 shadow-sm">
              <h2 className="text-sm font-semibold text-slate-900">Reportar una reparación</h2>
              <div className="mt-3 space-y-3">
                <label className="block text-sm">
                  ...
                  <input
                    ...
                    maxLength={120}
                    placeholder="Ej.: Teja rota que gotea"
                ...
                <label className="block text-sm">
                  ...
                  <textarea
                    ...
                    rows={3}
                    maxLength={1500}
                    placeholder="Cuenta qué pasa y desde cuándo."
                    className="w-full rounded-2xl border-2 border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-[#5646E5]"
                  />
                </label>
                <label className="block text-sm">
                  <span className="mb-1 block font-medium text-slate-600">Foto (opcional, JPG/PNG)</span>
                  <input
                    type="file"
                ...
                >
                  {busy ? "Enviando…" : "Reportar reparación"}
                </button>
          ...
          <section className="mt-6 space-y-3">
            <h2 className="text-sm font-semibold text-slate-900">Solicitudes</h2>
            ...
            {!loading && requests.length === 0 && (
              <p className="rounded-2xl border border-slate-200 bg-white/80 p-4 text-sm text-slate-600">
                Aún no hay solicitudes de reparación.
              </p>
            )}
            {requests.map((r) => {
                ...
                <article key={r.id} className="rounded-3xl border-2 border-slate-200 bg-white/90 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      <h3 className="text-[15px] font-bold text-[#17151F]">{r.title}</h3>
                  ...
                  {r.responses.length > 0 && (
                    <ul className="mt-2 space-y-1 text-[11px] text-slate-600">
                      {r.responses.map((resp, i) => (
                        <li key={i}>
                          {resp.action === "accept" ? "✅ Aceptada" : "❌ Rechazada"}
                  ...
                  {/* Disputa: ofrecer abogado aliado a ambas partes */}
                  {dispute && (
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    {!isLandlord && r.status === "rejected" && r.reportedByEmail === (user?.email ?? "") && (
                      <button
                        type="button"
                    ...
                    {active && r.reportedByEmail === (user?.email ?? "") && (
                      <button
                        type="button"
                  ...
                  {/* Motivo de rechazo inline (dueño) */}
                  {isLandlord && rejectingId === r.id && (
      ...
      {lawyerFor && juridica && (
        <ContactModal
          ...
          defaultMessage={`Tengo una disputa por una reparación ("${lawyerFor.title}") en mi arriendo y necesito asesoría jurídica.`}
