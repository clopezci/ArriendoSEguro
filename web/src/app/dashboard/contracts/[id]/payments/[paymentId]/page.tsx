...
export default function PaymentDetailPage() {
  ...
  return (
    <WizardShell title="Detalle de pago" currentStep={11} contractId={contractId}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="text-xs text-slate-700">
          Método
          <select className="mt-1 w-full rounded border border-slate-300 bg-white p-2 text-sm" value={paymentData.paymentMethod} onChange={(e) => setPayment((prev) => (prev ? { ...prev, paymentMethod: e.target.value } : prev))}>
      ...
      <label className="mt-2 block text-xs text-slate-700">
        Observaciones
        <textarea className="mt-1 min-h-24 w-full rounded border border-slate-300 bg-white p-2 text-sm" value={paymentData.notes ?? ""} onChange={(e) => setPayment((prev) => (prev ? { ...prev, notes: e.target.value } : prev))} />
      </label>
      <p className="mt-2 text-xs text-slate-600">Estado actual: {paymentData.paymentStatus}</p>
      <div className="mt-2 rounded border border-slate-300 p-3 text-xs text-slate-700">
        ...
        {paymentData.supportFileUrl ? (
          <a className="text-violet-700" href={paymentData.supportFileUrl} target="_blank" rel="noreferrer">
            Ver soporte
          </a>
        ) : null}
      </div>
      ...
      <div className="mt-5 rounded border border-amber-400 p-3">
        <p className="text-xs text-amber-800">Marcar como disputado</p>
        <input className="mt-1 w-full rounded border border-slate-300 bg-white p-2 text-sm" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Motivo de disputa" />
        <button type="button" onClick={markDisputed} disabled={saving || reason.trim().length < 3} className="mt-2 rounded border border-amber-500 px-3 py-2 text-xs text-amber-800">Marcar en revisión</button>
      </div>
      <div className="mt-5 rounded border border-slate-300 p-3">
        <p className="text-xs text-slate-700">Historial de cambios</p>
...
function Input({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <label className="text-xs text-slate-700">
      {label}
      <input type={type} className="mt-1 w-full rounded border border-slate-300 bg-white p-2 text-sm" value={value} onChange={(e) => onChange(e.target.value)} />
    </label>
  );
}
