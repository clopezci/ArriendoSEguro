...
- [ ] **Recordatorios al inquilino + escalamiento** (nuevo, QR): `POST https://arriendoseguro.app/api/payments/tenant-reminders/send-due` con header `Authorization: Bearer <CRON_SECRET>`. Prográmalo **1 vez al día**. Envía al inquilino el recordatorio **3 días antes** y **el día del vencimiento** con el QR/cuenta del dueño y el **enlace mágico** para subir su soporte; y **escala a ambas partes** los soportes que el dueño no confirmó en 3 días. Requiere correo del inquilino en el contrato y, para mostrar el QR/cuenta, que el dueño haya configurado el método de pago en «Adicionales → Método de pago y recordatorios».
- [ ] **Alertas de errores** (nuevo): `POST https://arriendoseguro.app/api/observability/error-alert/send-due` con header `Authorization: Bearer <CRON_SECRET>`. Prográmalo **cada 5–15 minutos**. Te envía correo a `ADMIN_INTERNAL_EMAILS` cuando hay errores recientes sin resolver por encima del **umbral** (por defecto **1**, configurable en `/admin` → «Estado y alertas»), con un período de enfriamiento para no saturar. La página pública de estado es **`/estado`** (no requiere cron).
