...
|-------|---------------|---------|-----------|
| 2026-09-22 | Claude Opus 4.8 | **Auditoría profunda del módulo Agencias (3 agentes) + correcciones.** Autorización/IDOR: **limpio**. Integridad de dinero corregida: **A1** liquidación de orden ATÓMICA (`claimOrderForSettlement` compartido por settler y webhook Wompi → sin doble acreditación por webhook duplicado/carrera con barrido); **B1** consumo de crédito por contrato atómico (`consumeCreditForContract`: lifecycle+saldo en una transacción → sin doble cobro por send-signatures concurrente); **B2** candado por correo en auto-registro (`agency_signup_locks` → sin farmeo de prueba multi-IP); **B3** auto-recarga no se bloquea si se abandona el checkout (orden pendiente >24h vence). Seguridad: **SSRF** `validateOutboundUrl` (https+bloqueo de IPs internas/metadata) en estudio externo y webhook; **inyección HTML** escapada en correos de registro/inquiry; **rate-limit** en inquiry y verify-identity. Pendiente de decisión: **B4** (tope legal 1% se omite si el avalúo es desconocido en el flujo de agencia). Probado end-to-end en prod (registro+dedup) previo. | main `81e446c` |
