...
|-------|---------------|---------|-----------|
| 2026-09-23 | Claude Opus 4.8 | **Limitador de alcance de IA por punto + más proveedores.** La IA conversacional (`/api/nuevo/assist` ask y `/api/legal/ask`) solo responde temas de arriendo/ArriendoSeguro/legal del arriendo; off-topic (cultura general, etc.) → **mensaje predeterminado** vía token centinela (`lib/ai/topicGuard.ts`), con alcance amplio para no bloquear lo legítimo (saludos y dudas del arriendo pasan). Verificado en prod con casos reales. Tareas acotadas (extract/moderación/respuesta a reseña/visión) sin limitador. Cadena de IA ampliada: **Groq→Gemini→OpenRouter (gratis)→OpenAI→DeepSeek (pago, último)**; env nuevas opcionales `OPENROUTER_API_KEY`/`DEEPSEEK_API_KEY` (redeploy). | main `413e642`, `8c75bba` |
| 2026-09-22 | Claude Opus 4.8 | **Auditoría profunda del módulo Agencias (3 agentes) + correcciones.** Autorización/IDOR: **limpio**. Integridad de dinero corregida: **A1** liquidación de orden ATÓMICA (`claimOrderForSettlement` compartido por settler y webhook Wompi → sin doble acreditación por webhook duplicado/carrera con barrido); **B1** consumo de crédito por contrato atómico (`consumeCreditForContract`: lifecycle+saldo en una transacción → sin doble cobro por send-signatures concurrente); **B2** candado por correo en auto-registro (`agency_signup_locks` → sin farmeo de prueba multi-IP); **B3** auto-recarga no se bloquea si se abandona el checkout (orden pendiente >24h vence). Seguridad: **SSRF** `validateOutboundUrl` (https+bloqueo de IPs internas/metadata) en estudio externo y webhook; **inyección HTML** escapada en correos de registro/inquiry; **rate-limit** en inquiry y verify-identity. **B4** (tope legal 1% cuando no hay avalúo): resuelto con **checkbox de aceptación** humana en generar/lote (main `765e24b`) — ya no se omite en silencio. Build `next build` limpio. | main `81e446c`, `765e24b` |
| 2026-09-22 | Claude Opus 4.8 | **Agencias — auto-registro con prueba (sin aprobación manual):** formulario público `/agencias/registro` → `POST /api/agencias/registro` (rate-limit `leads`) crea la agencia YA con `origin:self_signup` + `trial` y 3 créditos gratis (`AGENCY_TRIAL_CREDITS`); al admin solo le llega **aviso informativo** (correo + Telegram), no aprueba. El admin **revoca** la prueba desde `/admin` → Agencias (badge "en prueba", filtro "solo en prueba", "Revocar prueba" con mensaje que se envía a la agencia por correo vía `notifyMessage` en el PATCH). Campo **dudas/solicitudes** en la pestaña Créditos (`POST /api/agency/[id]/inquiry` → `agency_inquiries` + correo/Telegram). Nuevos campos `Agency.origin/trial/suspendedMessage`. Templates `agencyTrial*`. Typecheck limpio. | main `6309605` |
| 2026-06-05 | Claude Code | **Modelo monetización/firma — referido calificado + SMS solo pago:** dominio `referrals` con `QUALIFIED_REFERRALS_FOR_SIGNATURE_UNLOCK=3`, `signatureUnlockedByReferrals`, `countQualifiedReferrals` y micropago $10k. Endpoint `api/referrals/mark-usage` (autenticado): marca al referido como **calificado SOLO si usó la app de verdad** (verifica server-side que existe una versión de contrato con su correo como arrendador). La vista previa lo dispara al guardar versión. `api/referrals/me` devuelve `signatureUnlock` (X/3 + desbloqueado); `ReferralPanel` muestra el progreso. **SMS confirmado solo-pago** (novedades exige Plus/demo; cron renovación solo procesa firmados; E3 es solo correo) + comentarios explícitos. +2 tests (152/152). Decisión guardada en memoria. | `(este)` |
| 2026-06-05 | Claude Code | **Pulidos post-epic:** (1) el **certificado de pagos** excluye los pagos subidos por el inquilino que el dueño aún **no confirmó** (`isCountablePayment`: solo cuentan si `ownerConfirmed` o si no vinieron por el enlace mágico); rutas CSV/PDF mapean los campos. (2) **Progreso del wizard** arreglado: el progreso lineal cubre solo el **Bloque 1** (9 pasos contiguos, sin el salto de cláusulas); las páginas de adicionales/posventa usan `WizardShell variant="extra"` (encabezado simple, sin barra). 150/150. Lighthouse: pendiente de correr por el fundador (checklist listo). | `(este)` |
| 2026-06-04 | Claude Code | **Epic QR/2-bloques — Etapas E3 + E4 (CIERRE DEL EPIC):** **E4)** confirmación del dueño en **1 clic** desde el correo (`api/payments/confirm`, token en el `payments_log`): confirma/rechaza, marca el pago y **avisa al inquilino** (`paymentConfirmedToTenantEmail`); el aviso `paymentUploadedEmail` trae confirmar/rechazar. **E3)** cron `api/payments/tenant-reminders/send-due` (CRON_SECRET, 1×día): recordatorio al inquilino **3 días antes** y **el día del vencimiento** con QR/cuenta + enlace mágico (`tenantPaymentReminderEmail`), y **escalamiento a ambas partes** si el dueño no confirma en 3 días (`ownerConfirmEscalationEmail`). Manual: programar el cron. Fix `LOOKUP_CONSENTS_COLLECTION` fuera del route. 149/149. **Epic QR/2-bloques COMPLETO (A–E).** | `(este)` |
| 2026-06-04 | Claude Code | **Epic QR/2-bloques — Etapa E2 (enlace mágico del inquilino):** dominio `paymentUploadToken` + `lib/payments/uploadTokenStore` (token por periodo, TTL 45d, reusa el activo). El dueño genera el enlace desde la página de pagos (`api/payments/upload-link`). Página pública **`/pago/[token]`** (sin login): muestra cómo pagar (QR/cuenta del dueño) y permite **subir el soporte** (`upload/info`, `upload/sign`, `upload/submit`) a Storage real; crea el registro de pago **pendiente de confirmación del dueño** y le avisa por correo (`paymentUploadedEmail`). +5 tests (149/149). Pendiente E3 (recordatorios 3d/día) y E4 (confirmación 1-clic + escalamiento). | `(este)` |
| 2026-06-04 | Claude Code | **Epic QR/2-bloques — Etapa D + E1:** **D)** Centro de adicionales `/adicionales` (Bloque 2: codeudores, garantía, cláusulas, notaría, documentos, método de pago); Bloque 1 más corto (servicios→resumen; cláusulas opcionales desde el hub; codeudor diferible con "configurar luego"). **E1)** página `pagos-recordatorios`: el dueño elige **cuenta / QR / ninguno** con **consentimiento** explícito (dominio `paymentSettings`, rutas `payment-settings` GET/PUT y `payment-qr/{upload-url,download-url}`). Sin recaudo ni custodia. +6 tests (144/144). Pendiente E2–E4 (enlace mágico del inquilino, recordatorios 3d/día, confirmación 1-clic + escalamiento). | `(este)` |
